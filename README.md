...
The browser connects with a bare `io()` (same origin — no URL or CORS). Session
state is an in-memory `Map` on the server, which is intentional for a
single-instance app. Full design notes — folder structure, UI decisions per
breakpoint, component architecture, and the realtime event flow — are in
